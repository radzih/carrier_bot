...
def finded_stations_text(i18n: I18nMiddleware):
    return i18n.gettext(
        'Оберіть з переліку нижче станцію прибуття 👇'
    )
...
def not_finded_stations_text(i18n: I18nMiddleware):
    return i18n.gettext(
        ...
        'Або оберіть з переліку нижче станцію прибуття 👇\n'
    )
...
def register_finded_station_handlers(dp: Dispatcher):
    dp.register_message_handler(
        ...
        state=[
            states.SelectTicket.get_start_station,
            states.SelectTicket.get_end_station,
        ],
    )
