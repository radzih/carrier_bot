...
async def send_tickets(
    ...
    for ticket_id in tickets_ids:
        ...
        await bot.send_message(
            ...
            text=(
                '<b>Тип</b>: Квиток\n'
                f'<b>ПІБ</b>: {ticket.passenger.name} {ticket.passenger.surname}\n'
                f'<b>Час купівлі</b>: {datetime.datetime.now()}\n'
                f'<b>Дата відправлення</b>: {ticket.departure_time.strftime("%d.%m.%Y")}\n'
                f'<b>Час відправлення</b>: {ticket.departure_time.strftime("%H:%M")}\n'
                f'<b>Дата прибуття</b>: {ticket.arrival_time.strftime("%d.%m.%Y")}\n'
                f'<b>Час прибуття</b>: {ticket.arrival_time.strftime("%H:%M")}\n'
                f'<b>Станція відправлення</b>: {ticket.start_station.full_name}\n'
                f'<b>Станція прибуття</b>: {ticket.end_station.full_name}\n'
                f'<b>Вартість</b>: {ticket.price} грн\n'
                f'<b>Знижка у відсотках</b>: {ticket.type.discount}%\n'
                f'<b>Оплачено</b>: {"Так" if ticket.is_paid else "Ні"}\n'
                f'<b>Категорія</b>: {ticket.type.name}\n'
                '\n'
                '<b>Замовник</b>:\n'
                f'<b>ПІБ</b>: {ticket.owner.full_name}\n'
                f'<b>Номер телефону</b>: {ticket.owner.phone}\n'
            )
        )
